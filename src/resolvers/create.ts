...
export function createHandlerResolver(resolverMetadata: BaseResolverMetadata): GraphQLFieldResolver<any, any, any> {
  const { validate: globalValidate, authChecker, authMode, pubSub, globalMiddlewares, container } = BuildContext
  ...
  return (root, args, context, info) => {
    ...
    const targetInstanceOrPromise: Promise<any> | any = container.getInstance(resolverMetadata.target, resolverData)
    if (isPromiseLike(targetInstanceOrPromise)) {
      return targetInstanceOrPromise.then(async targetInstance =>
    ...
    return applyMiddlewares(container, resolverData, middlewares, () => {
      const params: Promise<any[]> | any[] = getParams(resolverMetadata.params!, resolverData, globalValidate, pubSub)
...
export function createBasicFieldResolver(fieldMetadata: FieldMetadata): GraphQLFieldResolver<any, any, any> {
