...
describe('MetadataStorage', () => {
  describe('resolvers inheritance', () => {
    ...
    beforeAll(async () => {
      ...
      function createAbstractResolver(classType: ClassType): any {
    ...
    it('should not have duplicated query metadata for inherited resolvers', async () => {
      expect(getMetadataStorage().queries.filter(query => query.schemaName === INHERITED_QUERY_NAME)).toHaveLength(1)
    ...
    it('should not have duplicated mutation metadata for inherited resolvers', async () => {
      expect(
        getMetadataStorage().mutations.filter(mutation => mutation.schemaName === INHERITED_MUTATION_NAME)
      ).toHaveLength(1)
